    ...
    for col in required_cols:
        if df[col].count() > 10:
            ...
            if len(clean_data) > 0 and clean_data.std() > 0:
                z_scores = np.abs(
                    stats.zscore(clean_data)
                )  # stats imported later in app? We'll import scipy.stats at top of matching? Actually we'll import scipy.stats in data_loader? Better to import in this function. We'll add import scipy.stats at top.
