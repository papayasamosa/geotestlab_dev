...
def run_matching(
    ...
    if setup_mode == "Set test/control constraints":
        ...
        if len(test_geos) == 0:
            raise ValueError(
                "Could not construct a valid experiment group with the provided constraints."
            )
        guided_share_info = {
    ...
    scaler, w_vec, p_scaled, t_cent = preprocess_data(
        pool_df, test_df_run, active_features, weights
    )
    opt_data = []
    best_smd = float("inf")
    best_idx = None
    ...
    iterations = (
        CONFIG["genetic_iterations"]["default"]
    ...
    for n in size_range:
        if match_mode == "Greedy (Nearest Neighbor)":
            ...
            m, _, _, _, _ = calculate_metrics(
                test_df_run, agg_df.loc[c_idx], active_features, weights
            )
            ...
            curr_smd, _, _, _, _ = calculate_metrics(
                test_df_run, agg_df.loc[curr_idx], active_features, weights
            )
            conv = [curr_smd]
            improved = True
            while improved:
                ...
                for j in range(min(len(curr_idx), 5)):
                    for swap_in in pot_swaps[:10]:
                        temp = curr_idx.copy()
                        temp[j] = swap_in
                        n_smd, _, _, _, _ = calculate_metrics(
                            test_df_run, agg_df.loc[temp], active_features, weights
                        )
            ...
            for _ in range(actual_iterations):
                ...
                n_smd, _, _, _, _ = calculate_metrics(
                    test_df_run, agg_df.loc[cand], active_features, weights
                )
                if n_smd < curr_smd:
