...
def placebo_analysis(uplift_list, real_uplift):
    ...
    z_score = (
        (real_uplift - np.mean(uplift_arr)) / np.std(uplift_arr)
        if np.std(uplift_arr) > 0 and real_uplift is not None
        else np.nan
    )
...
def run_validation_method(
    ...
    smapes_ro, rmses_ro = rolling_origin_validation(
        X_pre, y_pre, horizon=4, n_splits=5, model_type=method_name
    )
    ...
    if placebos and uplift is not None:
        median_uplift, p2_5, p97_5, percentile_rank, p_one_sided, z_score = placebo_analysis(
            placebos, uplift
        )
        ...
        median_uplift = p2_5 = p97_5 = percentile_rank = p_one_sided = z_score = (
            median_placebo_smape
    ...
    if method_name == "lasso":
        ...
        selected_df = pd.DataFrame(
            [(r, coeff_dict[r]) for r in selected], columns=["Region", "Coefficient"]
        )
